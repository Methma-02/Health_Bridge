...
//           setGender(data.WeightGainData[0].gender);
//         }
//       }
...
const WeightGainChart = () => {
  ...
  const fetchDataByRegistrationNumber = async (regNoParam) => {
    ...
    try {
      ...
      // Check if WeightGainData exists and has measurements
      if (data.WeightGainData && data.WeightGainData.length > 0 && data.WeightGainData[0].measurements) {
        setFormData(prev => ({
          ...prev,
          regNo: regNo,
        ...
        // If no weight data exists, just update the registration number
        setFormData(prev => ({
          ...prev,
          regNo: regNo,
          chartPoints: []
      ...
      // Only show alert if called from button click (not from auto-load)
