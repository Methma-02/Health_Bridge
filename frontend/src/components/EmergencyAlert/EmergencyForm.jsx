...
const EmergencyForm = ({ onClose }) => {
  const [isConfirming, setIsConfirming] = useState(true); // Start in the confirmation state
  const [additionalInfo, setAdditionalInfo] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const { setActiveEmergency } = useEmergency();
  
  // This function is now removed since we're starting in the confirmation state
